...
def main():
    ...
    yaml_files = sorted(
        [f for f in glob.glob(os.path.join(args.src_dir, "*.yaml")) if "Real" not in f]
    )
    if not yaml_files:
    ...
    for N in sorted(method_stats["Proposed"].keys()):
        ...
        for method in ["Baseline", "Human"]:
            if method in method_stats and N in method_stats[method]:
                ...
                significant_results[N][method] = p_value < 0.05 and np.mean(
                    proposed_data
                ) > np.mean(other_data)

    # Prepare plot
    plt.figure(figsize=(10, 6))
    method_colors = {"Proposed": "red", "Baseline": "green", "Human": "yellow"}
    method_labels = {
        "Proposed": "SART (Ours)",
    ...
    for method in method_stats:
        ...
        mean_across_files = [
            np.mean(method_stats[method][N]["means"]) for N in Ns_sorted
        ]
    ...
    for N in sorted(significant_results.keys()):
        improvements = [
            method for method, is_better in significant_results[N].items() if is_better
        ]
        if improvements:
            print(
                f"N={N}: Proposed significantly better than {', '.join(improvements)}"
            )
        # Add star annotation if significantly better than both methods
        if len(improvements) == 2:
            plt.text(N, 100.0, "*", fontsize=20, ha="center")

    # Replay (horizontal blue line)
    if replay_means:
        plt.axhline(
            y=np.mean(replay_means),
            color="blue",
            linestyle="--",
            label="Single Demo Replay",
        )
